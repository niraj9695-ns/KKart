...
const ProductDetails = () => {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:8080/api/carts/${userId}/add`, // Adjust `5` to the actual userId
        {}, // Still send an empty object for the body since @RequestParam is used
        {
          params: requestData, // Pass query parameters in `params`
        }
      );
